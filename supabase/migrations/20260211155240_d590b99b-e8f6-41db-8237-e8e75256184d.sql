-- Add numero_acta column to submissoes_entidade
ALTER TABLE public.submissoes_entidade ADD COLUMN numero_acta text;

-- Update aceitar_submissao_entidade function to store the acta number
...
BEGIN
  ...
  -- Update status and store acta number
  UPDATE submissoes_entidade 
  SET status = 'aguarda_validacao_chefe', numero_acta = _numero_acta, atualizado_em = now() 
  WHERE id = _submissao_id;
